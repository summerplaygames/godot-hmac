...
int64_t ArrayMesh::surface_find_by_name(const String name) const {
	static godot_method_bind *mb = nullptr;
	if (mb == nullptr) {
		mb = godot::api->godot_method_bind_get_method("ArrayMesh", "surface_find_by_name");
	}
...
Error ArrayMesh::lightmap_unwrap(const Transform transform, const double texel_size) {
	static godot_method_bind *mb = nullptr;
	if (mb == nullptr) {
		mb = godot::api->godot_method_bind_get_method("ArrayMesh", "lightmap_unwrap");
	}
	return (Error) ___godot_icall_int_Transform_float(mb, (const Object *) this, transform, texel_size);
}
